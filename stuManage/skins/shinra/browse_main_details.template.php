...
<table id="toptable">
	<tr valign="bottom">
		...
		<td style="text-align: <?php echo __("left"); ?>;">
			<input type="text" name="directory" value="<?php echo $directory_html; ?>" style="width: 400px;display:none;" title="(accesskey g)" accesskey="g" /> 
			<!-- <?php printActionIcon("listdirectories", "createDirectoryTreeWindow('$directory_js','BrowseForm.directory');"); ?><br /> -->
			<span style="font-size: 80%;display:none;"><?php echo __("Directory Tree"); ?>: <?php echo $directory_tree; ?></span>
			<span>提示：您不是一定要使用该平台才能上传网站源代码。您还可以通过其他ftp客户端连接到服务器，服务器地址：student.aibcloud.org。<br/>推荐ftp客户端：8uftp</span>
		</td>
		...
		<td valign="middle" style="text-align: <?php echo __("left"); ?>;display:none;">
...
<?php				if ($net2ftp_settings["functionuse_advanced"]       == "yes") { ?><input type="button" id="smallbutton" value="<?php echo __("Advanced"); ?>"       onclick="submitBrowseForm('<?php echo $directory_js; ?>', '', 'advanced', 'main');"         title="<?php echo __("Go to the advanced functions"); ?> (accesskey n)" accesskey="n" /> <?php } // end if ?> -->
				...
				<!-- <?php echo __("Transform selected entries: "); ?> --><span>对文件进行: &nbsp;</span>
...
<?php				if ($net2ftp_settings["functionuse_zip"]            == "yes") { ?><input type="button" id="smallbutton" value="压缩为<?php echo __("Zip"); ?>"            onclick="submitBrowseForm('<?php echo $directory_js; ?>', '', 'zip', 'zip');"               title="<?php echo __("Zip the selected entries to save or email them"); ?> (accesskey z)" accesskey="z" /> <?php } // end if ?>
<?php				if ($net2ftp_settings["functionuse_unzip"]          == "yes") { ?><input type="button" id="smallbutton" value="解压zip"     onclick="submitBrowseForm('<?php echo $directory_js; ?>', '', 'unzip', '');"                title="<?php echo __("Unzip the selected archives on the FTP server"); ?> (accesskey i)" accesskey="i" /> <?php } // end if ?>
<!-- <?php echo __("Unzip"); ?>  在上面的value里-->  
<?php				if ($net2ftp_settings["functionuse_calculatesize"]  == "yes") { ?><input type="button" id="smallbutton" value="统计<?php echo __("Size"); ?>"           onclick="submitBrowseForm('<?php echo $directory_js; ?>', '', 'calculatesize', '');"        title="<?php echo __("Calculate the size of the selected entries"); ?> (accesskey q)" accesskey="q" /> <?php } // end if ?>
...
<?php /* ----- Statistics ----- */ ?>
<div style="font-size: 90%; margin-top: 10px; margin-left: 630px; display:none;">
